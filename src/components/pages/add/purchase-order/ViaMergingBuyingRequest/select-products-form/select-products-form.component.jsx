...
import BuyingRequestSectionList from './buying-request-section-list.component';
...
const SelectProductsForm = function ({
  ...
  const handleTransfer = function ({ productItemData }) {
    if (
      ...
      updateDefaultSelectedCount(selectedCount + 1);
      const foundIndex = [...selectedProducts].findIndex(function (item) {
        if (item?.productId === productItemData?.productId) {
          return item;
        }
      });
      updateSelectedProductItemData({
  ...
  const productItemOnClick = function ({ selected, productItemData }) {
    ...
    if (selected) {
      selectedProducts.push({
        buyingRequestId: productItemData?.buyingRequestId,
        productId: productItemData?.productId,
        sourceUomId: '',
        sourceQuantity: 1,
        destUomId: productItemData?.productUomId,
        destQuantity: productItemData?.quantity,
        priceBeforeTax: 0,
        taxPercentage: 0,
      });
      productInfoList.push(productItemData);
      setSelectedCount(selectedCount + 1);
      handleTransfer({ productItemData: productItemData });
    } else {
      const foundIndex = [...selectedProducts].findIndex(function (item) {
        if (item?.productId === productItemData?.productId) {
          return item;
  ...
  const continueBtnOnClick = function () {
    ...
    // console.info('selectedCount', selectedCount);
        ...
        <SimpleBarReact className='h-full w-full'>
          <div className='flex flex-col space-y-6 px-2.5 pt-1.5 pb-3'>
            <div className={'flex flex-col w-full'}>
              <BuyingRequestSectionList
                accessToken={accessToken}
                setErrorMsg={setErrorMsg}
                setViewErrorPopup={setViewErrorPopup}
                viewErrorPopup={viewErrorPopup}
                productItemOnClick={productItemOnClick}
                defaultSelectedList={productInfoList}
                defaultBuyingRequestInfoList={defaultBuyingRequestInfoList}
              />
            </div>
          </div>
        </SimpleBarReact>
