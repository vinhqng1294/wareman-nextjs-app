import { InputState } from '@/components/ui/input/input.enum';
import InputDate from '@/components/ui/input/simple-input-date.component';
import InputText from '@/components/ui/input/simple-input-text.component';
import InputTextArea from '@/components/ui/input/simple-input-textarea.component';
import { isFunction } from '@/utils/commons/checkVariableType.utils';
import { stringIsNotEmpty } from '@/utils/commons/validateNotEmpty.utils';
import { SvgIcon } from '@/utils/global.enums';
import moment from 'moment';
import React, { useState } from 'react';
import { ReactSVG } from 'react-svg';
import SimpleBarReact from 'simplebar-react';
import FormHeader from './form-header.component';

const BasicInfoForm = function ({
  formHeader,
  handleNextStep,
  updateFinalData,
  defaultValue,
  ...props
}) {
  const [basicInfo, setBasicInfo] = useState({
    name: defaultValue?.name,
    description: defaultValue?.description,
    notes: defaultValue?.notes,
    // customDateTime: stringIsNotEmpty(defaultValue?.customDateTime)
    //   ? defaultValue?.customDateTime
    //   : moment(new Date()).format('DD/MM/YYYY'),
  });
  const [inputValidation, setInputValidation] = useState({
    name: true,
    description: true,
    notes: true,
    customDateTime: true,
  });
  const [inputFields, setInputFields] = useState({
    name: {
      label: 'Tên yêu cầu xuất kho',
      placeholder: '',
      errorText: 'Vui lòng nhập tên yêu cầu xuất kho',
    },
    description: {
      label: 'Mô tả',
      placeholder: '',
      errorText: '',
    },
    notes: {
      label: 'Ghi chú',
      placeholder: '',
      errorText: '',
    },
    customDateTime: {
      label: 'Ngày nhập kho',
      placeholder: 'DD/MM/YYYY',
      errorText: 'Vui lòng chọn ngày xuất kho',
    },
  });

  const continueBtnOnClick = function () {
    if (isFunction(handleNextStep) && isFunction(updateFinalData)) {
      updateFinalData({ data: { ...basicInfo } });
      handleNextStep({});
    }
  };

  const nameOnChange = function ({ value, evt }) {
    setBasicInfo({ ...basicInfo, name: value });
    if (stringIsNotEmpty(value)) {
      setInputValidation({ ...inputValidation, name: true });
    } else {
      setInputValidation({ ...inputValidation, name: false });
    }
  };
  const descriptionOnChange = function ({ value, evt }) {
    setBasicInfo({ ...basicInfo, description: value });
  };
  const notesOnChange = function ({ value, evt }) {
    setBasicInfo({ ...basicInfo, notes: value });
  };

  const customDateTimeOnChange = function ({ value, evt }) {
    setBasicInfo({ ...basicInfo, customDateTime: value });

    let selectedDate = moment(value, 'DD/MM/YYYY').format('YYYY-MM-DD');

    let currDate = new Date();
    currDate.setHours(23);
    currDate.setMinutes(59);
    currDate.setSeconds(59);
    currDate = moment(currDate).format('YYYY-MM-DD');

    let threeDaysAgo = new Date();
    threeDaysAgo.setDate(threeDaysAgo.getDate() - 3);
    threeDaysAgo.setHours(0);
    threeDaysAgo.setMinutes(0);
    threeDaysAgo.setSeconds(0);
    threeDaysAgo = moment(threeDaysAgo).format('YYYY-MM-DD');

    // console.info('selectedDate', selectedDate);
    // console.info('currDate', currDate);
    // console.info('threeDaysAgo', threeDaysAgo);

    if (stringIsNotEmpty(value)) {
      const isValid = moment(selectedDate).isBetween(
        threeDaysAgo,
        currDate,
        undefined,
        '[]'
      );
      setInputValidation({
        ...inputValidation,
        customDateTime: isValid,
      });

      const updatedInputFields = { ...inputFields };
      updatedInputFields.customDateTime.errorText = `Ngày nhập kho phải từ ngày ${moment(
        threeDaysAgo
      ).format('DD/MM/YYYY')} đến ngày ${moment(currDate).format(
        'DD/MM/YYYY'
      )}`;
      setInputFields({ ...inputFields, ...updatedInputFields });
    } else {
      setInputValidation({
        ...inputValidation,
        customDateTime: false,
      });

      const updatedInputFields = { ...inputFields };
      updatedInputFields.customDateTime.errorText =
        'Vui lòng chọn ngày nhập kho';
      setInputFields({ ...inputFields, ...updatedInputFields });
    }
  };

  return (
    <React.Fragment>
      <FormHeader title={formHeader?.title} subTitle={formHeader?.subTitle} />
      <div
        className={'flex flex-col w-full'
          .concat(' overflow-hidden')
          .concat(' xxs:my-4 md:my-5')}
      >
        <SimpleBarReact className='h-full w-full'>
          <div className='flex flex-col space-y-4 px-2.5 pb-3'>
            <div className='flex flex-col'>
              <label className={'mb-1 font-semibold text-base text-dark'}>
                {inputFields?.name?.label}
              </label>
              <InputText
                inputState={
                  !inputValidation?.name
                    ? InputState.danger
                    : InputState.default
                }
                placeholder={inputFields?.name?.placeholder}
                defaultText={basicInfo?.name}
                onChange={nameOnChange}
              />
              {!inputValidation?.name &&
                stringIsNotEmpty(inputFields?.name?.errorText) && (
                  <div className='flex space-x-2 items-center mt-1.5'>
                    <ReactSVG
                      src={SvgIcon['exclamation-sr']}
                      className='w-5 h-5 fill-red-500 flex-none'
                    />
                    <p className='text-red-500 text-base'>
                      {inputFields?.name?.errorText}
                    </p>
                  </div>
                )}
            </div>

            <div className='flex flex-col'>
              <label className={'mb-1 font-semibold text-base text-dark'}>
                {inputFields?.description?.label}
              </label>
              <InputTextArea
                inputState={
                  !inputValidation?.description
                    ? InputState.danger
                    : InputState.default
                }
                placeholder={inputFields?.description?.placeholder}
                defaultText={basicInfo?.description}
                rows={5}
                onChange={descriptionOnChange}
              />
              {!inputValidation?.description &&
                stringIsNotEmpty(inputFields?.description?.errorText) && (
                  <div className='flex space-x-2 items-center mt-1.5'>
                    <ReactSVG
                      src={SvgIcon['exclamation-sr']}
                      className='w-5 h-5 fill-red-500 flex-none'
                    />
                    <p className='text-red-500 text-base'>
                      {inputFields?.description?.errorText}
                    </p>
                  </div>
                )}
            </div>

            <div className='flex flex-col'>
              <label className={'mb-1 font-semibold text-base text-dark'}>
                {inputFields?.notes?.label}
              </label>
              <InputTextArea
                inputState={
                  !inputValidation?.notes
                    ? InputState.danger
                    : InputState.default
                }
                placeholder={inputFields?.notes?.placeholder}
                defaultText={basicInfo?.notes}
                rows={5}
                onChange={notesOnChange}
              />
              {!inputValidation?.notes &&
                stringIsNotEmpty(inputFields?.notes?.errorText) && (
                  <div className='flex space-x-2 items-center mt-1.5'>
                    <ReactSVG
                      src={SvgIcon['exclamation-sr']}
                      className='w-5 h-5 fill-red-500 flex-none'
                    />
                    <p className='text-red-500 text-base'>
                      {inputFields?.notes?.errorText}
                    </p>
                  </div>
                )}
            </div>

            {/* <div className='flex flex-col'>
              <label className={'mb-1 font-semibold text-base text-dark'}>
                {inputFields?.customDateTime?.label}
              </label>
              <InputDate
                inputState={
                  !inputValidation?.customDateTime
                    ? InputState.danger
                    : InputState.default
                }
                placeholder={inputFields?.customDateTime?.placeholder}
                defaultText={basicInfo?.customDateTime}
                onChange={customDateTimeOnChange}
              />
              {!inputValidation?.customDateTime &&
                stringIsNotEmpty(inputFields?.customDateTime?.errorText) && (
                  <div className='flex space-x-2 items-center mt-1.5'>
                    <ReactSVG
                      src={SvgIcon['exclamation-sr']}
                      className='w-5 h-5 fill-red-500 flex-none'
                    />
                    <p className='text-red-500 text-base'>
                      {inputFields?.customDateTime?.errorText}
                    </p>
                  </div>
                )}
            </div> */}
          </div>
        </SimpleBarReact>
      </div>
      <div
        className={'flex flex-col'
          .concat(' justify-between items-center')
          .concat(' xxs:mt-auto md:mt-5')
          .concat(' py-1 px-2.5')}
      >
        <button
          type='button'
          className={'flex items-center justify-center'
            .concat(' rounded-full')
            .concat(' bg-blue-500')
            .concat(' px-5 py-2')
            .concat(' hover:shadow-md')
            .concat(' space-x-3')
            .concat(' xxs:w-full md:w-2/3')
            .concat(
              inputValidation?.name &&
                stringIsNotEmpty(basicInfo?.name) 
                // &&
                // inputValidation?.customDateTime &&
                // stringIsNotEmpty(basicInfo?.customDateTime)
                ? ' cursor-pointer'
                : ' cursor-not-allowed opacity-50'
            )}
          disabled={
            !(
              inputValidation?.name &&
              stringIsNotEmpty(basicInfo?.name) 
              // &&
              // inputValidation?.customDateTime &&
              // stringIsNotEmpty(basicInfo?.customDateTime)
            )
          }
          onClick={function (evt) {
            evt?.preventDefault();
            evt?.stopPropagation();
            continueBtnOnClick();
          }}
        >
          <p className='text-white font-semibold text-base'>Tiếp tục</p>
          <ReactSVG
            src={SvgIcon['arrow-right']}
            className={'fill-white'.concat(' w-3.5')}
          />
        </button>
      </div>
    </React.Fragment>
  );
};

export default BasicInfoForm;
