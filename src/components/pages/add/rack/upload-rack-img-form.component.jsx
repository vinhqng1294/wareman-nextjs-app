...
const UploadRackImgForm = function ({
  formHeader,
  handleNextStep,
  submitFinalData,
  imgDataList,
  ...
  const [rackImgDataList, setRackImgDataList] = useState({
    ...imgDataList,
  });
  const [photoIds, setPhotoIds] = useState([]);

  const processingUploadPhotos = async function () {
    console.info('rackImgDataList', rackImgDataList);
    console.info('UploadRackImgForm finalData', finalData);
    const imgFileList = [...rackImgDataList?.rawDataList?.fileList];
    const imgBinaryDataList = [
      ...rackImgDataList?.rawDataList?.localImgPathList,
    ];

    for (let i = 0; i < imgFileList?.length; i++) {
      const imgFileData = imgFileList[i];
      await getRackPhotoSignedUrl({
        accessToken,
  ...
  const handleSubmitData = async function () {
    if (objectIsNotEmpty(rackImgDataList)) {
  ...
  const backBtnOnClick = function () {
    if (isFunction(handleNextStep) && isFunction(updateImgDataList)) {
      updateImgDataList({ ...rackImgDataList });
  ...
  const handleSelectedImg = function ({ data: imgDataList }) {
    ...
    setRackImgDataList({ ...newImgDataList });
  };
          ...
          <div className='flex flex-col space-y-4 px-2.5 pb-3'>
            <div className='flex flex-col w-full px-2'>
              <MultiImgUploadWithPreview
                ...
                imgFileList={rackImgDataList?.rawDataList?.fileList}
              />
            </div>
          </div>
        </SimpleBarReact>
...
export default UploadRackImgForm;
