...
const UploadImgForm = function ({
  ...
  const [providerImgDataList, setProviderImgDataList] = useState({
    ...imgDataList,
  });
  const [photoIds, setPhotoIds] = useState([]);

  const processingUploadPhotos = async function () {
    console.info('providerImgDataList', providerImgDataList);
    console.info('UploadImgForm finalData', finalData);
    const imgFileList = [...providerImgDataList?.rawDataList?.fileList];
    const imgBinaryDataList = [
      ...providerImgDataList?.rawDataList?.localImgPathList,
    ];

    for (let i = 0; i < imgFileList?.length; i++) {
      const imgFileData = imgFileList[i];
      await getProviderPhotoSignedUrl({
        accessToken,
        ...
        .catch(function (err) {
          console.error('err', err);
          console.error('err', err.response);
          setErrorMsg('Upload hình thất bại');
  ...
  const handleSubmitData = async function () {
    if (objectIsNotEmpty(providerImgDataList)) {
  ...
  const backBtnOnClick = function () {
    if (isFunction(handleNextStep) && isFunction(updateImgDataList)) {
      updateImgDataList({ ...providerImgDataList });
  ...
  const handleSelectedImg = function ({ data: imgDataList }) {
    ...
    setProviderImgDataList({ ...newImgDataList });
  };
          ...
          <div className='flex flex-col space-y-4 px-2.5 pb-3'>
            <div className='flex flex-col w-full px-2'>
              <MultiImgUploadWithPreview
                ...
                imgFileList={providerImgDataList?.rawDataList?.fileList}
              />
            </div>
          </div>
        </SimpleBarReact>
