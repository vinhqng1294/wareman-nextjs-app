...
import BasicProviderInfoForm from './basic-provider-info-form.component';
...
const AddProviderMain = function ({ ...props }) {
  ...
  const addProviderSteps = {
    ...
    2: {
      title: 'Địa chỉ',
      subTitle: 'Vui lòng thêm dịa chỉ của nhà cung cấp',
    },
    3: {
      title: 'Thông tin liên hệ',
      subTitle: 'Vui lòng thêm thông tin liên hệ của nhà cung cấp',
    },
    4: {
      title: 'Hình ảnh của nhà cung cấp',
      subTitle: 'Vui lòng tải lên các hình ảnh của nhà cung cấp',
    },
  };
  ...
  const submitFinalData = async function ({ submitData, ...props }) {
    addProvider({
      ...
      .then(function ({ data: resData }) {
        console.info('addProvider resData', resData);
        redirectTo('/providers/'.concat(resData?.data?.id));
      })
      .catch(function (err) {
    ...
    function () {
      if (arrayIsNotEmpty(userPermissions)) {
        ...
        const result = checkPermission({
          permissionKey: PermissionList.CREATE_PROVIDER.key,
        ...
        >
          {pageLoading ? (
            ...
            <React.Fragment>
              ...
              >
                {currStep === 1 && (
                  <BasicProviderInfoForm
                    formHeader={addProviderSteps[currStep]}
...
export default AddProviderMain;
