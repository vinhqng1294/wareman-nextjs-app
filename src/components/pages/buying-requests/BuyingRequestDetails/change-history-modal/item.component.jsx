            ...
            {stringIsNotEmpty(itemInfo?.status) ? (
              <p
                className={'text-sm font-semibold'.concat(
                  itemInfo?.status === BuyingRequestStatusList[6].key
                    ? ' text-orange-400'
                    : itemInfo?.status === BuyingRequestStatusList[2].key
                    ? ' text-sky-600'
                    : itemInfo?.status === BuyingRequestStatusList[3].key
                    ? ' text-blue-600'
                    : itemInfo?.status === BuyingRequestStatusList[4].key
                    ? ' text-green-500'
                    : itemInfo?.status === BuyingRequestStatusList[5].key
                    ? ' text-red-600'
                    : ' text-zinc-500'
                )}
              >
                {`${
                  BuyingRequestStatusList[
                    `${Object.keys(BuyingRequestStatusList).find(function (
                      ...
                      if (
                        itemInfo?.status === BuyingRequestStatusList[key].key
                      ) {
                        return key;
                      }
                    })}`
