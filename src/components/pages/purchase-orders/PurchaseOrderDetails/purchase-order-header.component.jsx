...
const PurchaseOrderHeader = function ({
  purchaseOrderInfo,
  accessToken,
  setErrorMsg,
  setViewErrorPopup,
  ...
  const handleEditOnClick = function () {
    redirectTo(`/edit/purchase-order/${purchaseOrderInfo?.id}`);
  };
      ...
      >
        <div className='flex w-full px-5 pt-4 pb-1.5'>
          <div className='flex items-center'>
            <TextEllipsis
              content={
                stringIsNotEmpty(purchaseOrderInfo?.id)
            ...
            <div className='flex justify-end'>
              <div className='flex space-x-1'>
                ...
                <p className={`text-sm text-zinc-500 font-medium`}>
                  {moment(purchaseOrderInfo?.updatedDate).format(
                    'DD/MM/YYYY HH:mm:ss'
                  )}
                </p>
              </div>
        ...
        >
          <div className='flex flex-col w-full'>
            <div className='flex'>
              <TextEllipsis
                content={
                  stringIsNotEmpty(purchaseOrderInfo?.name)
                    ? purchaseOrderInfo?.name
            ...
            <div className='flex space-x-2 mt-1'>
              ...
              <p className={`text-base text-blue-500 font-medium`}>
                {purchaseOrderInfo?.providerName}
              </p>
            </div>
            <div
            ...
            >
              <div className='flex space-x-2 w-full max-w-[24.5rem]'>
                ...
                <p className={`text-base text-blue-500 font-medium`}>
                  {makeFullName({
                    firstName: purchaseOrderInfo?.userFirstName,
                    lastName: purchaseOrderInfo?.userLastName,
                  })}
                </p>
              </div>
              <div className='flex space-x-2 w-full max-w-[24.5rem]'>
                ...
                <p className={`text-base text-blue-500 font-medium`}>
                  {moment(purchaseOrderInfo?.addedDate).format(
                    'DD/MM/YYYY HH:mm:ss'
                  )}
                </p>
              </div>
